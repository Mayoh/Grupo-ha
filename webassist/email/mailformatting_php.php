...
class WAUE_Log_Event {
	...
	function processEvent($isError) {
		...
		if (!function_exists("WA_AB_generateInsertParams")) require_once("../database_management/wa_appbuilder_php.php");
...
function writeUEProgress($EmailRef,$LoopCount,$TotalEmails,$TimeRemaining) {
	$UESendPath = getcwd();
	chdir(dirname(__FILE__));
	$myFile = $EmailRef . "_Progress_xml.php";
	$fh = @fopen($myFile, 'w');
	if ($fh) {
	  fwrite($fh, '<progress total="'.$TotalEmails.'" current="'.$LoopCount.'" remaining="'.$TimeRemaining.'" />');
	  fclose($fh);
	}
	chdir($UESendPath);
}
...
function WA_StripTags($bodytext)  {
  ...
  $bodytext = preg_replace("/\&nbsp\;?/"," ",$bodytext); 
